function pr::summarize::init --description "Set up a worktree to summarize a GitHub PR"
    if test (count $argv) -ne 1
        echo "Usage: pr::summarize::init <PR-URL|PR-NUMBER>" >&2
        return 2
    end
    ...
    set -l worktree_path $HOME/Projects/Worktrees/$rel/$pr_number-summarize
    ...
    if test -d $worktree_path
        echo "Worktree already exists — delegating to pr::summarize::pull."
        cd $worktree_path
        or return 1
        pr::summarize::pull
        return
